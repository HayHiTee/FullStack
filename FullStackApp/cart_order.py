...
class CartOrder:
    def create_order(self, total_amount, shipping_type, shipping_fee, shipping_region):
        trck_id = self._generate_tracking_id()
        while Orders.objects.filter(tracking_id=trck_id).exists():
            trck_id = self._generate_tracking_id()
        else:
            shipping = Shipping.objects.create(shipping_type=shipping_type, shipping_cost=shipping_fee,
